...
- 網路(硬體)設備: 集線器（hub） 交換器（switch） 路由器（router） 數據機（modem）
 - 集線器（hub）:指將多條乙太網路雙絞線或光纖集合連接在一起的裝置，把收到的任何數位訊號，經過再生或放大，再從所有埠送出，但造成訊號之間碰撞的機會很大，訊號也可能被竊聽，因此大部份集線器已被交換機取代，有點像中央節點 (central node) 的概念。
